          ...
          <div className="bg-surface rounded-[2.5rem] overflow-hidden shadow-card-hover border border-gray-200">
            <div className="grid lg:grid-cols-2 gap-0">
              <div className="p-8 sm:p-12 lg:p-16 flex flex-col justify-center">
                <span className="inline-flex text-xs font-bold bg-accent px-3 py-1 rounded-full w-fit mb-4 text-gray-900">Osiris for brands</span>
                <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold leading-[1.1]">Collaborate with top creators</h1>
                <p className="mt-4 text-gray-700 text-lg max-w-md leading-relaxed">Build your creator roster—faster. Shortlist creators with the right audience, launch campaigns, and measure ROI.</p>
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link href="/auth?tab=signup&role=brand" className="inline-flex items-center gap-2 bg-black text-white font-semibold text-sm px-6 py-3.5 rounded-full hover:bg-gray-800 transition-all shadow-md">Book a Demo</Link>
                </div>
              ...
              <div className="p-6 lg:p-8">
                <div className="grid grid-cols-3 grid-rows-2 gap-3 h-full min-h-[300px]">
                  {["bg-neon-blue","bg-pink-400","bg-amber-400","bg-emerald-400","bg-neon-purple","bg-neon-cyan"].map((bg, i) => (
                    <div key={i} className={`${bg} rounded-3xl ${i === 4 ? "col-span-2" : ""} shadow-md`}></div>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[{ icon: Search, t: "Discover", d: "Find the perfect creators for your brand with AI-powered search and detailed audience insights." },
                ...
                <div key={i} className="bg-surface rounded-3xl p-8 text-center border border-gray-200 shadow-card hover:shadow-card-hover transition-all">
                  <div className="w-12 h-12 rounded-xl bg-brand/15 mx-auto flex items-center justify-center mb-4"><f.icon className="w-6 h-6 text-brand" /></div>
