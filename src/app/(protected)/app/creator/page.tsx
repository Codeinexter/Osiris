...
export default function CreatorDashboard() {
  ...
  const myDeals = deals.filter((d) => d.creatorId === "1");
  ...
  const earnings = myDeals.filter((d) => ["active", "delivered", "paid"].includes(d.status)).reduce((s, d) => s + d.budget, 0);

  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold">Welcome back, {user?.name?.split(" ")[0] || "Creator"}</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: "Share public profile", desc: "Send your media kit link to potential brand partners.", href: "/c/priyafits", icon: Share2 },
        ].map((a) => (
          <Link key={a.href} href={a.href} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-brand/10 flex items-center justify-center shrink-0"><a.icon className="w-5 h-5 text-brand" /></div>
            <div className="flex-1"><h3 className="font-bold text-sm">{a.label}</h3><p className="text-xs text-gray-500 mt-0.5">{a.desc}</p></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/app/creator/deals" className="text-sm text-brand font-medium hover:underline">View all</Link>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 text-left text-gray-500">
              <th className="py-3 font-medium">Brand</th><th className="py-3 font-medium">Campaign</th><th className="py-3 font-medium">Status</th><th className="py-3 font-medium">Budget</th><th className="py-3 font-medium">Date</th>
            </tr></thead>
            <tbody>
              {myDeals.map((d) => (
                <tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="py-3"><TagPill label={d.status} type="status" /></td>
